/**
 * NexusCDP Mobile Browser SDK v1.0.0
 * Self-contained IIFE — no dependencies, no build step required.
 * IE11 compatible: uses XMLHttpRequest, no fetch, no arrow functions.
 * Default channel: 'mobile'. Extends base SDK with mobile-specific methods.
 */
(function (root, factory) {
  ...
  // Default channel for mobile
  var DEFAULT_CHANNEL = 'mobile';
  ...
  NexusCDP.init = function (options) {
    ...
    _log('initialized (mobile)', _config);
  ...
  /**
   * Track a custom event. Default channel is 'mobile'.
   */
  ...
  /**
   * Track app open — fires app_open event with platform and optional metadata.
   * @param {object} metadata - Any additional metadata (e.g. push campaign id, OS version).
   */
  NexusCDP.trackAppOpen = function (metadata) {
    NexusCDP.track('app_open', {
      platform: 'mobile',
      metadata: metadata || {}
    }, DEFAULT_CHANNEL, null);
  };
  ...
  /**
   * Track app background — fires app_background event.
   */
  NexusCDP.trackAppBackground = function () {
    NexusCDP.track('app_background', {
      platform: 'mobile'
    }, DEFAULT_CHANNEL, null);
  };

  /**
   * Biometric login — calls identify() with an extra authMethod:'biometric' trait.
   * @param {string} customerId
   * @param {object} traits - Additional traits merged with authMethod.
   * @param {function} callback
   */
  NexusCDP.biometricLogin = function (customerId, traits, callback) {
    var mergedTraits = {};
    if (traits) {
      for (var k in traits) {
        if (Object.prototype.hasOwnProperty.call(traits, k)) {
          mergedTraits[k] = traits[k];
        }
      }
    }
    mergedTraits.authMethod = 'biometric';
    NexusCDP.identify(customerId, mergedTraits, callback);
  };

  /**
   * Track a notification tap event.
   * @param {string} notificationId
   * @param {string} action - e.g. 'open', 'dismiss'
   * @param {function} callback
   */
  NexusCDP.trackNotificationTap = function (notificationId, action, callback) {
    NexusCDP.track('notification_tap', {
      notificationId: notificationId,
      action: action || 'open',
      platform: 'mobile'
    }, DEFAULT_CHANNEL, callback);
  };
